...
<script>
// ===== FOTOS RAPIDAS + FINALIZACION (script separado para evitar SyntaxError) =====
var _fotoAreas2 = [
    ...
    { key: 'bano', icon: '\uD83D\uDEBF', name: 'Bano' },
];
var _fotoIdx2 = 0;
var _limpId2 = {{ $tarea->apartamentoLimpieza ? $tarea->apartamentoLimpieza->id : 0 }};
var _csrf2 = '{{ csrf_token() }}';
...
function _actualizarFoto2() {
    ...
    var dots = document.querySelectorAll('#fotoDots .dot');
    for (var i=0;i<dots.length;i++) dots[i].style.color = i<=_fotoIdx2?'#0891b2':'#444';
}
...
function _enviarFinalizacionReal() {
    ...
    var fd = new FormData(document.getElementById('formPrincipalLimpieza'));
    fd.append('accion', 'finalizar');
    fd.set('consentimiento_finalizacion', 'true');
    fd.set('motivo_consentimiento', 'Finalizado con fotos rapidas');
    fetch(document.getElementById('formPrincipalLimpieza').action, {
        method:'POST', body:fd,
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    if(inp) inp.addEventListener('change',function(e){
        ...
        _comprimirImg2(file,1200,0.7).then(function(blob){
            ...
            fd.append('area',_fotoAreas2[_fotoIdx2].key);fd.append('_token',_csrf2);
            fetch('/gestion/limpieza/'+_limpId2+'/foto-rapida',{
                method:'POST',headers:{'X-CSRF-TOKEN':_csrf2},body:fd
            }).catch(function(err){console.error(err);});
        });
        setTimeout(function(){
            URL.revokeObjectURL(prev);_fotoIdx2++;
            if(_fotoIdx2>=5)_cerrarFotosYFinalizar();
            else{_actualizarFoto2();inp.value='';}
        },1000);
    });
});
</script>
...
<script>
// Inicializar limpieza ID para fotos rapidas (se carga desde /js/fotos-rapidas.js)
document.addEventListener('DOMContentLoaded', function() {
    if (typeof _initFotosRapidas === 'function') {
        _initFotosRapidas({{ $tarea->apartamentoLimpieza ? $tarea->apartamentoLimpieza->id : 0 }});
    }
});
</script>
@endsection

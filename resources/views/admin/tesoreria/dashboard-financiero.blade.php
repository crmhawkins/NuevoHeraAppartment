...
<div class="container-fluid py-2">
        ...
        <div class="col-8">
            <div class="card shadow-sm border-0">
                <div class="card-body py-2 px-3">
                    <p class="df-section-title mb-1"><i class="fas fa-chart-bar me-1" style="color:#0891b2;"></i>Ingresos (6 meses)</p>
                    <canvas id="chartIngresosMes" height="120"></canvas>
                </div>
            </div>
        </div>
        <div class="col-4">
            <div class="card shadow-sm border-0">
                <div class="card-body py-2 px-3">
                    <p class="df-section-title mb-1"><i class="fas fa-chart-pie me-1" style="color:#0891b2;"></i>Por Canal</p>
                    <canvas id="chartCanales" height="120"></canvas>
                </div>
            </div>
        </div>
    </div>

    {{-- Facturas pendientes antiguas --}}
    ...
    <div class="card shadow-sm border-0">
                ...
                <table class="table table-hover mb-0 df-table">
                    ...
                    @forelse($facturas as $f)
                        <tr>
                            ...
                            <td class="fw-bold">{{ number_format($f->total, 2, ',', '.') }}&euro;</td>
                            <td>
                                <select class="form-select form-select-sm estado-select" data-factura-id="{{ $f->id }}" style="width:100px;font-size:11px;padding:2px 4px;height:24px;">
        ...
        <div class="card-footer bg-white py-1" style="font-size:12px;">
...
<script>
    new Chart(document.getElementById('chartIngresosMes').getContext('2d'), {
        ...
        options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { font: { size: 10 }, callback: function(v) { return (v/1000).toFixed(0)+'k'; } } }, x: { ticks: { font: { size: 10 } } } } }
    });
    ...
    function cambiarEstado(id, estado) {
        fetch('/admin/tesoreria/factura/' + id + '/estado', {
            ...
            if (d.success && typeof Swal !== 'undefined') Swal.fire({ icon: 'success', title: d.message, timer: 1200, showConfirmButton: false });
        });
    }
</script>
@endsection
